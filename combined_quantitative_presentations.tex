...
\subsection{Estimation and the Proportional Hazards Assumption}
...
This approach assumes non-informative censoring.
\end{frame}

\begin{frame}{Checking the Proportional Hazards Assumption}
Several diagnostic methods assess the PH assumption:

\begin{block}{Graphical Methods}
\begin{itemize}
\item \textbf{Log-minus-log plots}: Parallel lines suggest PH holds
...
\begin{block}{Addressing Violations}
When PH is violated, consider stratified Cox models, time-dependent covariates, or parametric alternatives.
\end{block}
\end{frame}
...
\midrule
Epidemiological cohort studies & Identifying risk factors for disease \\
Clinical trials & Comparing treatment effects on survival \\
Vaccine effectiveness & Assessing protection over time \\
Health services research & Evaluating readmission patterns \\
\bottomrule
\end{tabular}
\end{center}
\end{frame}
...
\subsection{Step-by-Step PCA Implementation}
...
\subsection{Variance Explained and Component Selection}
...
\subsection{Interpretation of Principal Components}
...
\subsection{Phase 1: Problem Definition and Clinical Question}
...
Bridging the gap between clinical intuition and ML specifications:

\begin{itemize}
\item \textbf{Outcome Definition}: Precise operationalization of clinical endpoints (e.g., 30-day all-cause vs. preventable readmission).
\item \textbf{Prediction Window}: Determination of the relevant time horizon for clinical action (e.g., 24-hour vs. 7-day predictions).
\item \textbf{Threshold Selection}: Balance between sensitivity and specificity based on clinical consequences.
\end{itemize}
\end{frame}
...
\subsection{Phase 3: Feature Engineering and Selection}
...
\subsection{Phase 4: Model Selection and Training}
...
\subsection{Phase 5: Model Validation and Performance Evaluation}
...
\subsection{Phase 6: Deployment and Clinical Integration}
...
\subsection{Model Governance, Monitoring, and Updating}
...
\subsection{Data Privacy, Ethics, and Regulatory Considerations}
...
\subsection{Clinical Decision Support Examples}
