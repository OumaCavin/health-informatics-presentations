...
\begin{tikzpicture}[node distance=2cm, every node/.style={rectangle, rounded corners, minimum width=4cm, minimum height=1.2cm, text centered, draw=MidnightBlue, fill=blue!10}]
\node (data) {Observed Data\\$X_1, X_2, \ldots, X_n$};
\node (model) [right of=data, xshift=4cm] {Statistical Model\\$f(x|\theta)$};
\node (estimate) [below of=data, yshift=-1cm] {Estimator\\$\hat{\theta}$};

\draw [thick,->,>=stealth] (data) -- (model) node[midway, above] {Model assumption};
\draw [thick,->,>=stealth] (model) -- (estimate) node[midway, right] {MLE};
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[node distance=2cm, every node/.style={rectangle, rounded corners, minimum width=5cm, minimum height=1.5cm, text centered, draw=MidnightBlue, fill=blue!10}]
\node (origin) {Origin\\$t=0$};
\node (event) [right of=origin, xshift=5cm] {Event\\$T=t$};
...
\begin{tikzpicture}[node distance=2cm, every node/.style={rectangle, rounded corners, minimum width=4cm, minimum height=1.5cm, text centered, draw=MidnightBlue, fill=blue!10}]
\node (input) {Original Data\\$n \times p$ matrix\\$p = 15$ indicators};
\node (pca) [right of=input, xshift=5cm] {PCA\\Transformation};
\node (output) [right of=pca, xshift=5cm] {Reduced Data\\$n \times k$ matrix\\$k = 4$ components};

\draw [thick,->,>=stealth] (input) -- (pca) node[midway, above] {Linear\\projection};
\draw [thick,->,>=stealth] (pca) -- (output) node[midway, above] {Variance\\maximization};
\end{tikzpicture}
\end{frame}
