...
\setbeamercolor{footline}{fg=MidnightBlue, bg=LightSteelBlue}
\setbeamercolor{author in head/foot}{fg=MidnightBlue, bg=LightSteelBlue}
\setbeamercolor{date in head/foot}{fg=MidnightBlue, bg=LightSteelBlue}
\setbeamercolor{section in head/foot}{fg=MidnightBlue, bg=LightSteelBlue}
\setbeamercolor{subsection in head/foot}{fg=MidnightBlue, bg=LightSteelBlue}
\setbeamercolor{page number in head/foot}{fg=MidnightBlue, bg=LightSteelBlue}

% Custom footline with University of Nairobi, date and page number
\setbeamertemplate{footline}{
  \leavevmode%
  \hbox{
    \begin{beamercolorbox}[wd=.35\paperwidth,ht=8pt,dp=1pt,left]{author in head/foot}%
      \usebeamerfont{author in head/foot}\insertshortinstitute
    \end{beamercolorbox}%
    \begin{beamercolorbox}[wd=.35\paperwidth,ht=8pt,dp=1pt,center]{date in head/foot}%
      \usebeamerfont{date in head/foot}\insertdate
    \end{beamercolorbox}%
    \begin{beamercolorbox}[wd=.3\paperwidth,ht=8pt,dp=1pt,right]{page number in head/foot}%
...
\institute{University of Nairobi\\Department of Mathematics}
\date{\today}
...
\begin{frame}{Unit and Group Information}
\begin{tabular}{ll}
\toprule
\textbf{Unit} & \textbf{SDS6210 – Informatics for Health} \\
\midrule
Programme & MSc Public Health Data Science \\
Institution & University of Nairobi \\
Department & Department of Mathematics \\
\midrule
\textbf{Group 5 Members} & \\
...
\begin{center}
\begin{tikzpicture}[every node/.style={rectangle, rounded corners, minimum width=4cm, minimum height=1.2cm, text centered, draw=MidnightBlue, fill=blue!10}, node distance=2cm]
\node (data) {Observed Data};
\node (model) [right of=data, xshift=4cm] {Statistical Model};
\node (estimate) [below of=data, yshift=-1cm] {Estimator};

\draw [thick,->,>=stealth] (data) -- (model) node[midway, above] {$\{X_i\} \rightarrow f(x|\theta)$};
\draw [thick,->,>=stealth] (model) -- (estimate) node[midway, right] {MLE $\rightarrow \hat{\theta}$};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[node distance=1.5cm, every node/.style={rectangle, rounded corners, minimum width=10cm, minimum height=0.8cm, text centered}]
\node (I1) [draw=MidnightBlue, fill=blue!10] {$\beta_j > 0 \implies e^{\beta_j} > 1$: Increased odds of outcome};
\node (I2) [below of=I1, draw=Green, fill=green!10] {$\beta_j = 0 \implies e^{\beta_j} = 1$: No association};
\node (I3) [below of=I2, draw=Red, fill=red!10] {$\beta_j < 0 \implies e^{\beta_j} < 1$: Decreased odds of outcome};
\end{tikzpicture}
\end{frame}
...
\begin{center}
\begin{tikzpicture}[every node/.style={rectangle, rounded corners, text centered, draw=MidnightBlue, fill=blue!10}, node distance=2cm]
\node (origin) {Origin};
\node (event) [right of=origin, xshift=5cm, minimum width=3cm] {Event};

\draw[->, thick, >=stealth] (origin) -- (event) node[midway, above] {$T=$ time};
\node at (2.5, -1.5) [rectangle, draw=Green, fill=green!10, minimum width=3cm] {Observed};
...
\node [right] at (5,2.5) {Group A};
\node [right] at (5,1) {Group B};
\node [right] at (5.5,1.75) {HR = 1.8};
\node [right] at (5.5,1.35) {(Constant ratio)};
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[node distance=1.2cm, every node/.style={rectangle, rounded corners, minimum width=10cm, minimum height=0.9cm, text centered}]
\node (I1) [draw=Green, fill=green!10] {$\beta > 0 \implies \text{HR} > 1$: The covariate increases the hazard (risk factor)};
\node (I2) [below of=I1, draw=MidnightBlue, fill=blue!10] {$\beta = 0 \implies \text{HR} = 1$: No association};
\node (I3) [below of=I2, draw=Red, fill=red!10] {$\beta < 0 \implies \text{HR} < 1$: The covariate decreases the hazard (protective factor)};
\end{tikzpicture}
\end{frame}
...
\begin{center}
\begin{tikzpicture}[every node/.style={rectangle, rounded corners, text centered, draw=MidnightBlue, fill=blue!10}, node distance=2cm]
\node (input) {Original Data};
\node (pca) [right of=input, xshift=4cm] {PCA};
\node (output) [right of=pca, xshift=4cm] {Reduced Data};

\draw [thick,->,>=stealth] (input) -- (pca) node[midway, above] {Linear proj.};
\draw [thick,->,>=stealth] (pca) -- (output) node[midway, above] {Variance max.};
\end{tikzpicture}
\end{center}
\end{frame}
...
\node [right] at (3,2.5) {Data ellipse};
\node [right] at (1,1.8) {PC1 captures major};
\node [right] at (1,1.5) {axis variance};
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=1.5cm, every node/.style={rectangle, rounded corners, minimum width=10cm, minimum height=1cm, text centered}]
\node (C1) [draw=Red, fill=red!10] {High correlation among clinical indicators};
\node (C2) [below of=C1, draw=Orange, fill=orange!10] {Small sample sizes relative to number of variables};
\node (C3) [below of=C2, draw=Yellow, fill=yellow!10] {Multicollinearity complicates regression modeling};
\node (C4) [below of=C3, draw=MidnightBlue, fill=blue!10] {Difficult interpretation of 15+ correlated variables};
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[node distance=1.2cm, every node/.style={rectangle, rounded corners, minimum width=10cm, minimum height=0.9cm, text centered}]
\node (B1) [draw=MidnightBlue, fill=blue!10] {Dimensionality reduction: 15 $\rightarrow$ 4 variables (73\% reduction)};
\node (B2) [below of=B1, draw=Green, fill=green!10] {Decorrelation: Components are uncorrelated (eliminates multicollinearity)};
\node (B3) [below of=B2, draw=Orange, fill=orange!10] {Noise reduction: Low-variance components often capture measurement error};
\node (B4) [below of=B3, draw=Purple, fill=purple!10] {Interpretability: Each PC represents a clinical dimension};
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[node distance=1cm, every node/.style={rectangle, rounded corners, minimum width=12cm, minimum height=0.9cm, text centered}]
\node (S1) [draw=MidnightBlue, fill=blue!10] {\textbf{Step 1:} Standardize data: $Z_{ij} = (X_{ij} - \bar{X}_j) / s_j$};
\node (S2) [below of=S1, draw=Green, fill=green!10] {\textbf{Step 2:} Compute correlation matrix: $\mathbf{R} = \frac{1}{n-1}\mathbf{Z}^T\mathbf{Z}$};
\node (S3) [below of=S2, draw=Orange, fill=orange!10] {\textbf{Step 3:} Eigenvalue decomposition: $\mathbf{R}\mathbf{v}_j = \lambda_j\mathbf{v}_j$};
\node (S4) [below of=S3, draw=Purple, fill=purple!10] {\textbf{Step 4:} Order by eigenvalues: $\lambda_1 \ge \lambda_2 \ge \cdots \ge \lambda_{15}$};
\node (S5) [below of=S4, draw=Red, fill=red!10] {\textbf{Step 5:} Select top $k$ components and compute scores: $\mathbf{PC} = \mathbf{Z}\mathbf{V}_k$};
\end{tikzpicture}
\end{frame}
...
\draw[dashed, red] (0,1) -- (16,1) node[right] {Kaiser criterion ($\lambda=1$)};

\node[circle, draw=Red, fill=red!10, minimum size=0.3cm] at (4,1.68) {};

\node [right] at (10,4) {Scree Plot};
\node [right] at (10,3.5) {Elbow at PC4};
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[node distance=1cm, every node/.style={rectangle, rounded corners, minimum width=11cm, minimum height=0.9cm, text centered}]
\node (L1) [draw=Red, fill=red!10] {\textbf{Linearity}: PCA only captures linear relationships};
\node (L2) [below of=L1, draw=Orange, fill=orange!10] {\textbf{No class discrimination}: Components may not separate disease groups};
\node (L3) [below of=L2, draw=Yellow, fill=yellow!10] {\textbf{Interpretability}: Component meaning may be unclear};
\node (L4) [below of=L3, draw=MidnightBlue, fill=blue!10] {\textbf{Information loss}: Some variance is discarded};
\end{tikzpicture}
\end{frame}
...
\begin{frame}{References: Q4 - ML Workflow}
\begin{small}
\begin{itemize}
